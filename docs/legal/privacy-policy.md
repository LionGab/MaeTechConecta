# Política de Privacidade - Nossa Maternidade

> ⚠️ **TODO:** Este arquivo é um placeholder. Contratar advogado LGPD para criar documento completo.

## Informações Necessárias

Esta política deve conter:

1. **Dados Coletados**
   - Nome, email, tipo de usuário (gestante/mãe/tentante)
   - Dados de saúde (semana de gravidez, nome do bebê)
   - Histórico de conversas com IA
   - Dados de uso do app

2. **Base Legal (LGPD Art. 7)**
   - Consentimento explícito
   - Execução de contrato
   - Legítimo interesse

3. **Finalidade do Tratamento**
   - Personalização de conteúdo
   - Assistência via IA
   - Melhoria do serviço

4. **Compartilhamento de Dados**
   - Supabase (hospedagem)
   - Google Gemini (IA)
   - Serviços de terceiros (se houver)

5. **Direitos do Titular (LGPD Art. 18)**
   - Acesso aos dados
   - Correção de dados
   - Exclusão de dados
   - Portabilidade
   - Revogação de consentimento

6. **Segurança**
   - Medidas técnicas
   - Criptografia
   - Retenção de dados

7. **Contato DPO**
   - Nome do Encarregado de Dados
   - Email de contato

8. **Alterações**
   - Política de atualizações
   - Notificação de mudanças

## Próximos Passos

- [ ] Contratar advogado especializado em LGPD
- [ ] Criar documento completo
- [ ] Revisar com equipe jurídica
- [ ] Hospedar em: `https://nossamaternidade.com.br/privacidade`
- [ ] Adicionar link no app.json
- [ ] Adicionar link nas telas do app

---

**Status:** ⚠️ PENDENTE  
**Criado em:** 2025-11-06  
**Responsável:** [Definir]

