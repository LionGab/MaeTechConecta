...
- **Aliases internos**: `gemini-2.5-flash` e `gemini-2.5-pro` continuam aceitos e são resolvidos automaticamente para os modelos *exp*.

## Mapping e GeminiError
A função `getGeminiEndpointForModel(model)` converte nomes internos ou *exp* para o endpoint oficial (`/models/<modelo>:generateContent`). Modelos desconhecidos disparam `GeminiError` com código `unknown` – trate no serviço chamador para definir fallback ou logar o incidente.
