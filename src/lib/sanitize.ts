 ...
 */
export function sanitizeObject<T extends Record<string, unknown>>(obj: T, maxDepth: number = 10): T {
  if (maxDepth <= 0) {
    return obj;
  }
  ...
  for (const [key, value] of Object.entries(obj)) {
    if (typeof value === 'string') {
      ...
      (sanitized as Record<string, unknown>)[key] = sanitizeObject(value as Record<string, unknown>, maxDepth - 1);
