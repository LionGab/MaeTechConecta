...
const MEDICAL_URGENCY_KEYWORDS = [
  ...
  'dor forte', 'muita dor', 'dor insuportável', 'dor intensa',
  'desmaio', 'desmaiei', 'tontura extrema',
  'febre alta', 'febre acima de 38',
  'convulsão',
  'contrações antes da hora',
  'não sinto o bebê mexer', 'bebê parou de mexer',
  'água rompeu', 'bolsa rompida',
  'pressão alta',
  'visão embaçada', 'vendo pontinhos',
];

const PSYCHOLOGICAL_KEYWORDS = [
  'não vejo sentido', 'não quero mais viver', 'quero morrer',
  'pensar em me machucar', 'pensamentos suicidas',
  'muito desesperada', 'sem esperança',
  'chorando sem parar', 'não consigo parar de chorar',
  'medo excessivo', 'pânico',
  'não consigo comer', 'perdi completamente o apetite',
  'insônia total',
];
...
serve(async (req) => {
  try {
    ...
    if ((classification.medicalRisk >= 7 || classification.psychologicalRisk >= 7) && userId) {
      ...
      const riskType = classification.medicalRisk >= classification.psychologicalRisk
        ? 'medical'
        : 'psychological';
