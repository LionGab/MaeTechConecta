-- Schema Completo Final - NathIA Semana 1
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE c.relkind='r' AND n.nspname='public' AND c.relname='chat_messages') THEN
...
-- Função para atualizar contador de interações
...
-- Status
SELECT 'Schema criado/atualizado com sucesso!' as status;
